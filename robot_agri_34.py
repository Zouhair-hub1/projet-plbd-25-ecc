...
def desherber():
    s1 = S1_TRAVAIL
    print("\n*** DESHERBAGE : centrage objet rouge ***")
...
def trajet():
    global desherbage_actif, detection_flag, t_fin_desherbage
    DUREE_MISSION = 7 * 60  # 7 minutes
    debut_mission = time.time()

    while time.time() - debut_mission < DUREE_MISSION:
        # === INITIALISATION DU CYCLE (comme premier demarrage) ===
        print("\n========== DEBUT CYCLE ==========")
        ...
        # === AVANCE EN CONTINU ===
        print("Robot avance... (live actif, detection active)")
        sc.set_angle(0, 90)
        time.sleep(0.3)
        move.Motor(1, 1, 5)
        move.Motor(2, 1, 5)

        # Attente detection ou fin de mission
        detected = False
        while time.time() - debut_mission < DUREE_MISSION:
            with detection_lock:
        ...
        # === DETECTION ! ===
        move.motorStop()
        desherbage_actif = True  # Cadres YOLOv8 ignores des maintenant
        print("Mauvaise herbe detectee !")
        print("Modele desactive (pas de cadres dans le live)")
        time.sleep(2)
        ...
        # === PHOTO VERIFICATION ===
        print("Capture photo de verification...")
        sc.set_angle(1, S1_TRAVAIL)
        sc.set_angle(2, S2_TRAVAIL)
        time.sleep(1)
        ret, frame = camera.read()
        if ret:
            ...
            try:
                requests.post(f'http://{PC_IP}:5000/save_verification',
                              ...
                              timeout=5)
                print("Photo envoyee a verification arrachage !")
            except Exception as e:
                print(f"Erreur envoi photo : {e}")
        time.sleep(2)

        # === POSITION INITIALE ===
        print("Retour position initiale (tous servos 90)...")
        position_initiale()

        # === ATTENTE 15s (modele toujours desactive) ===
        print("Attente 15s (live visible mais pas de cadres)...")
        time.sleep(15)

        # === REACTIVATION (comme un redemarrage complet) ===
        t_fin_desherbage = time.time()
        desherbage_actif = False  # Cadres YOLOv8 reactives
        try:
            requests.get(f'http://{PC_IP}:5000/activer_detection', timeout=2)
            print("Detection reactivee ! Nouveau cycle...")
        except:
            pass
